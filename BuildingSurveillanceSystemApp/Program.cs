namespace BuildingSurveillanceSystemApp
{
    class Program
    {
    ...
    public class EmployeeNotify : Observer
    {
        ...
        public override void OnCompleted()
        {
            ...
            foreach (var externalVisitor in _externalVisitors)
            {
                ...
                Console.WriteLine($"{externalVisitor.Id,-6}{externalVisitor.FirstName,-15}{externalVisitor.LastName,-15}{externalVisitor.EntryDateTime.ToString("dd-MM-yyyy hh:mm:ss"),-25}{externalVisitor.ExitDateTime.ToString("dd-MM-yyyy hh:mm:ss tt"),-25}");
            }
            Console.WriteLine();
            Console.WriteLine();
        }
        ...
        public override void OnNext(ExternalVisitor value)
        {
            ...
            if (externalVisitor.EmployeeContactId == _employee.Id)
            {
                ...
                if (externalVisitorListItem == null)
                {
                    ...
                    Console.WriteLine($"{_employee.FirstName + " " + _employee.LastName}, your visitor has arrived. Visitor ID({externalVisitor.Id}), FirstName({externalVisitor.FirstName}), LastName({externalVisitor.LastName}), entered the building, DateTime({externalVisitor.EntryDateTime.ToString("dd-MM-yyyy hh:mm:ss")})");
